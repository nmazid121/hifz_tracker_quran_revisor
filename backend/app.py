...
def get_surah_names():
    """Get mapping of surah numbers to Arabic and English names."""
    try:
        surah_names = {
            "1": "الفاتحة",
            "2": "البقرة", 
...
def get_page_layout(page_number):
    """Get page layout data formatted according to QUL specification."""
    try:
        data = cached_get_page(page_number)
        if not data['pageData']:
            return jsonify({'error': 'Page not found'}), 404
        
        # Return in the exact format specified in the requirements
        return jsonify({
            'pageLayout': data['pageData'],  # Array of line objects from pages table
            'wordData': transform_word_map(data['wordData'])  # Object/map where keys are word_index and values are word text
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
